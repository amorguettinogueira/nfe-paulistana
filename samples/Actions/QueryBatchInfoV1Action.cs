...
/// <summary>
/// Consulta informações de um lote RPS via serviço SOAP <c>ConsultaInformacoesLote</c> V01
/// (<see cref="Nfe.Paulistana.V1.Services.IConsultaInformacoesLoteService"/>).
/// <para><b>Configuração necessária (<see cref="Configuration.AppSettings"/>):</b>
/// <c>MinhaInscricaoMunicipal</c>, <c>MeuCnpj</c>;
/// <c>ConsultaInformacoesLoteV1:NumeroLote</c> é opcional —
...
/// </summary>
internal sealed class QueryBatchInfoV1Action(PedidoInformacoesLoteFactory factory,
                                             IConsultaInformacoesLoteService service,
                                             AppSettings settings,
                                             ILogger<QueryBatchInfoV1Action> logger) : IIntegrationAction
{
    public int MenuOrder => ActionCatalog.QueryBatchInfoV1.Order;
    public string Description => ActionCatalog.QueryBatchInfoV1.Description;

    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            var numeroLote = settings.ConsultaInformacoesLoteV1.NumeroLote > 0
                ? (Numero?)settings.ConsultaInformacoesLoteV1.NumeroLote.Value
                : null;
        ...
        {
            ConsolePrinter.PrintException(ex, logger, "Erro em ConsultaInformacoesLoteV1Action");
        }
    }
}