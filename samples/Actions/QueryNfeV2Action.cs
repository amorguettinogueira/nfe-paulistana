...
/// <c>ConsultaNFe</c> V02 (<see cref="Nfe.Paulistana.V2.Services.IConsultaNFeService"/>).
/// <para><b>Configuração necessária (<see cref="Configuration.AppSettings"/>):</b>
/// <c>MeuCnpj</c>, <c>MinhaInscricaoMunicipal</c>,
/// <c>ConsultaNfeV2:NumeroNotaFiscal</c>.</para>
/// <para><b>Efeitos:</b> chamada de rede SOAP com mTLS.
/// Idempotente — somente leitura no servidor da Prefeitura.</para>
/// </summary>
internal sealed class QueryNfeV2Action(PedidoConsultaNFeFactory factory,
                                       IConsultaNFeService service,
                                       AppSettings settings,
                                       ILogger<QueryNfeV2Action> logger) : IIntegrationAction
{
    public int MenuOrder => ActionCatalog.QueryNfeV2.Order;
    public string Description => ActionCatalog.QueryNfeV2.Description;
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            ConsolePresenter.Info($"  Número da Nota Fiscal: {settings.ConsultaNfeV2.NumeroNotaFiscal}");

            DetalheConsultaNFe[] detalhes = [
                new(new ChaveNfe(
                    inscricaoPrestador: (InscricaoMunicipal)settings.MinhaInscricaoMunicipal,
                    numeroNFe: (Numero)settings.ConsultaNfeV2.NumeroNotaFiscal)),
            ];
        ...
        {
            ConsolePrinter.PrintException(ex, logger, "Erro inesperado em ConsultaNFeV2Action");
        }
    }
}