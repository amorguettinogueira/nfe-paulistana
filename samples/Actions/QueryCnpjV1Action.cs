...
/// <c>ConsultaCNPJ</c> V01 (<see cref="Nfe.Paulistana.V1.Services.IConsultaCNPJService"/>).
/// <para><b>Configuração necessária (<see cref="Configuration.AppSettings"/>):</b>
/// <c>MeuCnpj</c>, <c>ConsultaCnpjV1:CnpjAConsultar</c>.</para>
/// <para><b>Efeitos:</b> chamada de rede SOAP com mTLS.
/// Idempotente — somente leitura no servidor da Prefeitura.</para>
/// </summary>
internal sealed class QueryCnpjV1Action(PedidoConsultaCNPJFactory factory,
                                        IConsultaCNPJService service,
                                        AppSettings settings,
                                        ILogger<QueryCnpjV1Action> logger) : IIntegrationAction
{
    public int MenuOrder => ActionCatalog.QueryCnpjV1.Order;
    public string Description => ActionCatalog.QueryCnpjV1.Description;
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            ConsolePresenter.Info($"  CNPJ a Consultar: {settings.ConsultaCnpjV1.CnpjAConsultar}");

            var pedidoConsulta = factory.NewCnpj(
                cnpj: (Cnpj)settings.MeuCnpj,
                cnpjContribuinte: (CpfOrCnpj)(Cnpj)settings.ConsultaCnpjV1.CnpjAConsultar);
        ...
        {
            ConsolePrinter.PrintException(ex, logger, "Erro em ConsultaCnpjV1Action");
        }
    }
}