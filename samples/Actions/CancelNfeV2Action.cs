...
/// <summary>
/// Cancela uma NF-e de serviços via serviço SOAP <c>CancelamentoNFe</c> V02
/// (<see cref="Nfe.Paulistana.V2.Services.ICancelamentoNFeService"/>).
/// <para><b>Configuração necessária (<see cref="Configuration.AppSettings"/>):</b>
/// <c>MinhaInscricaoMunicipal</c>, <c>MeuCnpj</c>,
/// <c>CancelamentoNfeV2:NumeroNFeParaCancelar</c>.</para>
...
/// </summary>
internal sealed class CancelNfeV2Action(PedidoCancelamentoNFeFactory factory,
                                        ICancelamentoNFeService service,
                                        AppSettings settings,
                                        ILogger<CancelNfeV2Action> logger) : IIntegrationAction
{
    public int MenuOrder => ActionCatalog.CancelNfeV2.Order;
    public string Description => ActionCatalog.CancelNfeV2.Description;
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            ConsolePresenter.Info($"  Número da NF-e para Cancelar: {settings.CancelamentoNfeV2.NumeroNFeParaCancelar}");

            DetalheCancelamentoNFe[] detalhes = [
                new(new ChaveNfe(
                    inscricaoPrestador: (InscricaoMunicipal)settings.MinhaInscricaoMunicipal,
                    numeroNFe: (Numero)settings.CancelamentoNfeV2.NumeroNFeParaCancelar))
            ];
            ...
            ConsolePresenter.Outcome($"\nSucesso: {retorno.Cabecalho?.Sucesso} - NF-e cancelada: {settings.CancelamentoNfeV2.NumeroNFeParaCancelar}", retorno.Cabecalho?.Sucesso ?? false);
        ...
        {
            ConsolePrinter.PrintException(ex, logger, "Erro ao cancelar NF-e (V02)");
        }
    }
}